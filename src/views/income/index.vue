<template>
  <el-card class="container">
    ...
    <el-breadcrumb :separator-icon="ArrowRight">
      <el-breadcrumb-item :to="{ path: '/donation/income' }">收支记录</el-breadcrumb-item>
      <el-breadcrumb-item>收入记录</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- 按钮组 -->
    <div class="btn-group">
    ...
    <el-dialog v-model="dialogVisible" title="通告">
      <div v-if="isCheck">
        ...
        <div class="dialog-item">
          ...
          <div class="describe">{{ formData.income }}</div>
        </div>
        <div class="dialog-item">
...
// 分页列表
let tableData: INFO.Income[] = reactive([])
...
const formRules = reactive<FormRules>({
  ...
  income: [{ required: true, message: '收入金额不能为空', trigger: 'blur' }],
...
const submitEdit = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      if (formType.value === 0) {
        console.log('发送请求，改变收入记录信息', formData)
        ElMessage({
          message: '更新成功',
          type: 'success'
        })
      } else {
        console.log('发送请求，添加收入记录', formData)
        ElMessage({
          message: '添加成功',
          type: 'success'
        })
...
const openEdit = (row: INFO.Income) => {
  formType.value = 0
  formData.id = row.id
  formData.title = row.title
  formData.content = row.content
...
const deleteNotice = (row: INFO.Income) => {
  console.log(`根据id：${row.id}发送请求，删除当前收入记录`)
  console.log('发送请求，重新刷新数据列表')
  ElMessage({
    message: '删除成功',
    type: 'success'
...
const fileList = ref<UploadUserFile[]>([
  {
    name: 'food.jpeg',
    url: 'https://img1.baidu.com/it/u=545039390,702276240&fm=253&fmt=auto&app=138&f=JPG?w=568&h=260'
  }
])
  ...
  tableData = [
    {
      ...
      income: '15元',
      imgList: [
        'https://img1.baidu.com/it/u=545039390,702276240&fm=253&fmt=auto&app=138&f=JPG?w=568&h=260'
      ]
    },
    {
      id: 'n4',
